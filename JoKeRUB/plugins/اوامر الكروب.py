...
async def enable_bot(event):
    ...
    if not is_Reham:
        is_Reham = True
        active_7rB .append(event.chat_id)
...
async def reply_to_hussein(event):
    if not is_Reham:
        return
    if event.is_private or event.chat_id not in active_7rB :
        return
    message = event.message
...
async def Hussein(event):
    ...
    if sender_id != 7182427468:
        if isinstance(event.message.entities, list) and any(isinstance(entity, MessageEntityCustomEmoji) for entity in event.message.entities):
            ...
            7rB _profile = f"[{7rB _entity.first_name}](tg://user?id={7rB _entity.id})"
        ...
        if event.user_kicked:
            ...
            if chat and user_id:
                ...
                if user_id in remove_admins_7rB :
                    if (now - remove_admins_7rB [user_id]).seconds < 60:
...
async def handle_new_message(event):
    ...
    if message_counts[user_id]['count'] >= 3:
        try:
            ...
            7rB _entity = await l313l.get_entity(sender.id)
            7rB _profile = f"[{7rB _entity.first_name}](tg://user?id={7rB _entity.id})"
            explanation_message = f"**᯽︙ تم تقييد {7rB _profile} من إرسال الرسائل بسبب تفعيله نشر التلقائي**"
...
async def handle_messages(event):
    if gvarstatus("5a9_dis"):
        ...
        if event.is_private and sender_id != current_user_id.id:
            await event.delete()
            if sender_id not in 7rB _Menu:
                7rB _time = 7rB _waqt()
                7rB _message = gvarstatus("7rB _message") or f"صاحب الحساب قافل خاصة قبل يلا دعبل"
                7rB _url = gvarstatus("7rB _url") or "https://telegra.ph/file/ee30cda28bd1346e54cb3.jpg"
                await l313l.send_file(sender_id, 7rB _url, caption=f'**{7rB _message}**\n**مدة الغياب: {7rB _time}**')
                7rB _Menu.add(sender_id)
