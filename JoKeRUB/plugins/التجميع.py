...
async def _(event):
    ...
    for i in range(100):
        ...
        if "اشترك فالقناة @" in msg_text:
            7rB _channel = msg_text.split('@')[1].split()[0]
            try:
                entity = await l313l.get_entity(7rB _channel)
                if entity:
...
async def w3d_joker(event):
    ...
    while its_Reham:
        if event.is_group:
            ...
            await asyncio.sleep(3)
            7rB  = await event.client.get_messages(event.chat_id, limit=1)
            7rB  = 7rB [0].message
            7rB  = ("".join(7rB .split(maxsplit=2)[2:])).split(" ", 2)
            l313l = 7rB [0]
