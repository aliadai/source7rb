...
from ..helpers import get_user_from_event  # يُستخدم في أوامر أخرى عند الحاجة
...
async def get_user_from_event_emoji(event):
    """جلب المستخدم لـ .ايدي:
    - إن كان هناك رد → صاحب الرسالة.
    - إن وُجِد نص بعد الأمر → يُعتبر معرف/ايدي.
    - إن لم يوجد شيء → حسابك أنت.
    """

    # 1) في حال الرد على رسالة
    ...
    # 2) في حال كتابة معرف/ايدي بعد الأمر
    user = (event.pattern_match.group(1) or "").strip()
    if user:
        if user.isnumeric():
            user = int(user)
        try:
    ...
    # 3) بدون رد وبدون نص → رجّع معلومات نفسك
    try:
...
✸ **الرتبَه: [🫶](emoji/5764920531660837314) {rotbat} [🫶](emoji/5767030090747614223) [🛠](emoji/5215392879320505675)**
✸ **النبذة: {user_bio}**
——————————
""".strip().format(
        full_name=full_name,
...
async def ايدي_ايموجي_معلومات(event):
    ...
    replied_user = await get_user_from_event_emoji(event)
    try:
