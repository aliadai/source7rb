...
# Global variables for stopping all repeating operations
yaAli = False
global_stop_all = False
client = l313l
Mukrr = Config.MUKRR_ET or "مكرر"

async def spam_function(event, JoKeRUB, l313l, sleeptimem, sleeptimet, DelaySpam=False):
    global global_stop_all

    counter = int(l313l[0])
    if len(l313l) == 2:
        spam_message = str(l313l[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None or global_stop_all:
                return
...
async def hrb_nshr(l313l, sleeptimet, chat, message, seconds):
    global yaAli, global_stop_all
    yaAli = True
    while yaAli and not global_stop_all:
        if message.media:
...
async def Hussein(event):
    ...
    for chat_username in chat_usernames:
        try:
            ...
            await hrb_nshr(l313l, seconds, chat.id, message, seconds)  # تمرير قيمة seconds هنا لكل مجموعة
...
async def hrb_allnshr(l313l, sleeptimet, message):
    global yaAli
    yaAli = True
    hrb_chats = await l313l.get_dialogs()
    while yaAli:
...
async def Hussein(event):
    ...
    global yaAli
    yaAli = True
    await hrb_allnshr(l313l, sleeptimet, message)
super_groups = ["super", "سوبر"]
async def hrb_supernshr(l313l, sleeptimet, message):
    global yaAli
    yaAli = True
    hrb_chats = await l313l.get_dialogs()
    while yaAli:
...
async def Hussein(event):
    ...
    global yaAli
    yaAli = True
    await hrb_supernshr(l313l, sleeptimet, message)
@l313l.ar_cmd(pattern="ايقاف (النشر|نشر)")
async def stop_hrb (event):
    global yaAli
    yaAli = False
    await event.edit("**᯽︙ تم ايقاف النشر التلقائي بنجاح ✓** ")

# أمر الإيقاف الشامل - يوقف جميع العمليات المتكررة من أي مكان
@l313l.ar_cmd(pattern="ايقاف الكل$")
async def stop_all_operations(event):
    global yaAli, global_stop_all
    
    # إيقاف جميع العمليات
    yaAli = False
    global_stop_all = True
    delgvar("spamwork")
    
    await event.edit("**🛑 تم إيقاف جميع العمليات المتكررة بنجاح ✅**\n**📍 تم إيقاف: التكرار، النشر، السبام، وجميع العمليات الأخرى**")
    
    # إعادة تعيين المتغير العام بعد ثانيتين للسماح بعمليات جديدة
    await asyncio.sleep(2)
    global_stop_all = False

@l313l.ar_cmd(pattern="ستوب$")
async def stop_all_short(event):
    global yaAli, global_stop_all
